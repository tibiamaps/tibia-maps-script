...
import {
	serializeMarker,
	serializeMarkers,
} from '../src/serialize-markers.mjs';
...
if (serializedSingle !== expectedSingle) {
	console.error(
		'Error: `serializeMarker` output does not match expected format!',
	);
	process.exitCode = 1;
}
...
if (serializedSample !== expectedSample) {
	console.error(
		'Error: `serializeMarkers` output does not match expected format!',
	);
	process.exitCode = 1;
}
...
if (actualMarkersContent !== expectedMarkersContent) {
	console.error(
		'Error: `data/markers.json` content does not match expected format!',
	);
	process.exitCode = 1;
}
...
if (markers.length > 0) {
	console.error(
		'Error: `--no-markers` extracted marker data anyway! (data-without-markers/markers.json)',
	);
	process.exitCode = 1;
}
const emptyMarkersContent = readFile(
	'data-without-markers/markers.json',
).toString('utf8');
if (emptyMarkersContent !== '[]\n') {
	console.error(
		'Error: `data-without-markers/markers.json` content does not match expected format!',
	);
	process.exitCode = 1;
}
...
await fs.copyFile(
	'data-union-base/markers.json',
	'data-union-new/markers.json',
);
runCli(
	'--union --markers-only --from-minimap=minimap --output-dir=data-union-new',
);
compareMarkerFiles('data-union');
const actualUnionMarkersContent = readFile(
	'data-union-new/markers.json',
).toString('utf8');
const expectedUnionMarkersContent = readFile(
	'data-union/markers.json',
).toString('utf8');
if (actualUnionMarkersContent !== expectedUnionMarkersContent) {
	console.error(
		'Error: `data-union-new/markers.json` content does not match `data-union/markers.json`!',
	);
	process.exitCode = 1;
}
...
await fs.writeFile(
	'data/markers-sort-test.json',
	JSON.stringify(unsortedMarkers),
);
await sortMarkersInFile('data/markers-sort-test.json');
const actualDirectSortedContent = (
	await fs.readFile('data/markers-sort-test.json')
).toString('utf8');
if (actualDirectSortedContent !== expectedMarkersContent) {
	console.error(
		'Error: `sortMarkersInFile` output does not match expected format!',
	);
	process.exitCode = 1;
}
...
const actualCliSortedContent = (
	await fs.readFile('data/markers-sort-test.json')
).toString('utf8');
if (actualCliSortedContent !== expectedMarkersContent) {
	console.error(
		'Error: `tibia-maps --sort-markers` output does not match expected format!',
	);
	process.exitCode = 1;
}
await fs.unlink('data/markers-sort-test.json');
...
await fs.writeFile(
	'data-sort-dir-test/markers.json',
	JSON.stringify(unsortedMarkers),
);
runCli('--sort-markers=data-sort-dir-test');
const actualDirSortedContent = (
	await fs.readFile('data-sort-dir-test/markers.json')
).toString('utf8');
if (actualDirSortedContent !== expectedMarkersContent) {
	console.error(
		'Error: `tibia-maps --sort-markers` with directory argument output does not match expected format!',
	);
	process.exitCode = 1;
}
